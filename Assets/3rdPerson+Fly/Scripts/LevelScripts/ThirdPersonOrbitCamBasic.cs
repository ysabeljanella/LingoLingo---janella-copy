...
public class ThirdPersonOrbitCamBasic : MonoBehaviour 
{
    ...
    void Update()
	{
        ...
        if (useTouchInput && Input.touchCount > 0)
        {
            ...
            {
                // Check if the touch is within the bounds of the defined UI element
                if (RectTransformUtility.RectangleContainsScreenPoint(touchAreaRectTransform, touch.position))
                {
                    // Calculate horizontal and vertical movement based on touch delta position
